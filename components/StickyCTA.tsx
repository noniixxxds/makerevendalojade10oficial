...
export const StickyCTA: React.FC<StickyCTAProps> = ({ onAction }) => {
  return (
    <div className="fixed bottom-0 left-0 w-full bg-white border-t border-brand-light shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] p-3 z-50 md:hidden flex items-center justify-between px-4">
      ...
      <button 
        onClick={onAction}
        className="bg-[#25D366] hover:bg-green-600 text-white font-bold py-3 px-6 rounded-full shadow-lg animate-heartbeat flex items-center gap-2 text-sm justify-center border-2 border-green-400"
      >
